...
size_t N = 10;
double dt = 0.1;
int num_latency_states = 1; // 0.1 / 0.1
...
//
MPC::MPC() :
		a_(0), steering_delta_(0) {
	pred_waypoint_x_.resize(N - 1);
	pred_waypoint_y_.resize(N - 1);
}
MPC::~MPC() {
}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
	...
	for (i = delta_start; i < a_start; i++) {
		vars_lowerbound[i] = -DEGREE_LIMIT;
		vars_upperbound[i] = DEGREE_LIMIT;
	...
	// Steering angle is constant in lateency steps
	for (i = delta_start; i < delta_start + num_latency_states; i++) {
		vars_lowerbound[i] = steering_delta_;
		vars_upperbound[i] = steering_delta_;
	}

	// Throttle is constant in latency steps
	for (i = a_start; i < a_start + num_latency_states; i++) {
	...
	steering_delta_ = solution.x[delta_start + num_latency_states];
	a_ = solution.x[a_start + num_latency_states];

	for (i = 1; i < N; i++) {
		pred_waypoint_x_[i - 1] = solution.x[x_start + i];
		pred_waypoint_y_[i - 1] = solution.x[y_start + i];
	}
